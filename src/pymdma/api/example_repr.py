...
from ..constants import DataModalities, ReferenceType, ValidationTypes
...
def prepare_input_layer(
    ...
    if reference_type != ReferenceType.NONE:
        reference_data = Path(f"{_CONTAINER_DATA_ROOT}/{data_modality.value}/{validation_type.value}/reference")
    else:
        reference_data = None

    target_data = Path(f"{_CONTAINER_DATA_ROOT}/{data_modality.value}/{validation_type.value}/dataset")

    if annotation_file is not None:
        assert validation_type == ValidationTypes.INPUT, "Annotations are only supported for input validation"
        annotation_file = Path(
            f"{_CONTAINER_DATA_ROOT}/{data_modality.value}/{validation_type.value}/annotations/{annotation_file}",
        )
